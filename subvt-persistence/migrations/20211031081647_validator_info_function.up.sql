...
BEGIN
	...
	if is_active_param then
		...
		SELECT EXISTS(
			SELECT 1
			FROM block B, event_validator_heartbeat_received E
			WHERE E.block_hash = B.hash
			AND E.validator_account_id = account_id_param
			AND B.epoch_index IN (
				SELECT epoch_index
				FROM block
				WHERE block.hash = block_hash_param
			)
