...
impl PostgreSQLAppStorage {
    pub async fn get_network_by_id(&self, id: u32) -> anyhow::Result<Network> {
        Ok(sqlx::query_as(
            r#"
            SELECT id, hash, name, ss58_prefix, token_ticker, token_decimal_count, redis_url, network_status_service_url, report_service_url, validator_details_service_url, active_validator_list_service_url, inactive_validator_list_service_url
            FROM app_network
            WHERE id = $1
            "#
        )
